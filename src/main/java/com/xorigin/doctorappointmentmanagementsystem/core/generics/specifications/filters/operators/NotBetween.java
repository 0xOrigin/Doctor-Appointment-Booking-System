...
public class NotBetween extends AbstractFilterOperator {
    ...
    public Predicate apply(Path<?> path, CriteriaBuilder cb, List<?> values) {
        ...
        if (isTemporalFilter(path)) {
            ...
            return cb.not(cb.between(expression, jdbcTypes.getFirst(), jdbcTypes.getLast()));
        }

        return cb.not(cb.between(path.as(Comparable.class), (Comparable) values.getFirst(), (Comparable) values.getLast()));
    }
